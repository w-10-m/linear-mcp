...
describe('Server Lifecycle', () => {
  ...
  describe('startup', () => {
    ...
    it('starts with custom environment variables', async () => {
      await client.start({
        LINEAR_API_KEY: 'custom-test-key',
  ...
  describe('tools listing', () => {
    ...
    it('includes expected Linear tools', async () => {
      await client.start();
      ...
      expect(toolNames).toContain('linear_create_issue');
      expect(toolNames).toContain('linear_list_issues');
      expect(toolNames).toContain('linear_list_teams');
    });
  });
});
