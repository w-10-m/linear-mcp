...
describe('Config', () => {
  ...
  describe('loadConfig', () => {
    ...
    it('should load linear configuration', () => {
      process.env.LINEAR_API_KEY = 'lin_api_key';

      const config = loadConfig();

      expect(config.linear.lINEARAPIKEY).toBe('lin_api_key');
    });
  });
  ...
  describe('validateConfig', () => {
    ...
    it('should require LINEAR_API_KEY', () => {
      const config: ServerConfig = {
        logShipping: {
          enabled: false,
          endpoint: '',
      ...
      expect(result.errors).toContain('LINEAR_API_KEY environment variable is required for linear');
    });
