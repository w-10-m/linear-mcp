/**
 * LogShipper - Centralized log shipping service for linear-mcp
...
export class LogShipper {
  ...
  private validateAndCleanLogs(logs: LogEntry[]): LogEntry[] {
    return logs.map(log => {
      ...
      const cleanedLog: LogEntry = {
        ...
        serverName: log.serverName || 'linear-mcp',
  ...
  private generateSessionId(): string {
    return `linear-mcp-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
  }
  ...
  private logLocally(level: string, message: string, metadata?: any): void {
    ...
    console.error(`[LINEAR-MCP-LOG-SHIPPER] ${JSON.stringify(logEntry)}`);
  }
