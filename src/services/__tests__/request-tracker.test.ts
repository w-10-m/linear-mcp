...
describe('RequestTracker', () => {
  ...
  describe('cancelRequest', () => {
    ...
    it('should return false for already cancelled request', () => {
      tracker.registerRequest('req-1');
      tracker.cancelRequest('req-1');

      // Re-register since cancel cleans up
      tracker.registerRequest('req-2');
      tracker.cancelRequest('req-2');
      const result = tracker.cancelRequest('req-2');
  ...
  describe('cleanupStaleRequests', () => {
    it('should clean up requests older than maxAge', () => {
      ...
      // Manually set startTime to make it stale
      (context as any).startTime = Date.now() - 400000; // 400 seconds ago

      const cleanedCount = tracker.cleanupStaleRequests(300000); // 5 minute max age
