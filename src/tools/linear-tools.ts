  ...
  async executeTool(name: string, args: any, context?: RequestContext, progressReporter?: ProgressReporter): Promise<any> {
    ...
    if (!this.canHandle(name)) {
      this.logger.error('TOOL_ERROR', 'Unknown tool requested', {
        tool: name,
        supportedTools: ['linear_create_issue', 'linear_get_issue', 'linear_update_issue', 'linear_delete_issue', 'linear_list_issues', 'linear_list_teams', 'linear_get_team', 'linear_list_users', 'linear_get_user', 'linear_assign_issue', 'linear_create_project', 'linear_get_project', 'linear_list_projects', 'linear_update_project', 'linear_list_workflow_states', 'linear_transition_issue', 'linear_create_label', 'linear_list_labels', 'linear_apply_labels', 'linear_search_issues', 'linear_get_viewer', 'linear_create_comment', 'linear_list_comments']
      });
